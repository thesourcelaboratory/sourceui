...
function PreventDelete() {
	...
	this.preventdelete_nedit_class = 'nedt'
	//this.preventdelete_yedit_class = 'yedt'
	...
	this.check = function(node) {
		return $(node).is('.'+self.preventdelete_nedit_class);
	}
	this.checkParents = function(node) {
		if (!node) return true
		return $(node).closest('.'+self.preventdelete_nedit_class).length > 0;
	}
	this.checkChildren = function(node) {
		if (!node) return false
		return $(node).find('.'+self.preventdelete_nedit_class).length > 0;
	}
