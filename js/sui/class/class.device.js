...
sourceui.Device = function(c){
	...
	var Debug = function(){
		...
		var debug = this;
		var profiles = {};
		var isEnable = (Config.debug() && (Fingerprint.lier === true || Agent.data.browser.name == 'Chrome'));

		if (isEnable){
			/*
			window.onerror = function (m,u,l,c,e) {
				var str = m.toLowerCase();
				if (str.indexOf("script error") > -1) m = 'Script Error: See Browser Console for Detail';
				var html = '<p class="error icon-delete22">'+
				(m ? '<b>'+m+'</b>' : '')+
				(e ? '<span>'+e+'</span>' : '')+
				(u ? '<small>'+u+'</small>' : '')+
				(l||c ? '<small> ('+l+','+c+')</small>' : '')+
				'</p>';
				$('#suiBugModal > .container').prepend(html);
				var $badge = $('#suiBugBadge');
				var $mark = $badge.find('mark');
				$mark.text(parseInt($mark.text())+1);
				$('#suiBody').addClass('error');
				$badge
				.velocity({scale:1.55},'ease-out',250)
				.velocity({scale:1},'ease-in',200);
				return false;
			};
			*/
		}
